...
// copy the arguments to the global parameter variables
	...
	if ((func->name) && !strncmp(func->name, "CPPVectorAdd", 12))
	{
		printf("test\n");
	}
		...
		if (PR_Check("["))
		{
			...
			else
			{
				while (!PR_Check("]"))
				{
					...
					if (pr_token[0] != ',')
					{
						vec->push_back(pr_globals + def->ofs);
					}
				}
			}
			continue;
			...
			else if (PR_Check("["))
			{
				for (;;)
				{
					if (pr_token[0] == ']')		// Missi: empty vector (9/17/2023)
					{
						...
						memcpy((void*)(pr_globals + def->ofs), vec, sizeof(vec));
					}

					def->initialized = 1;
					def->type = type;
					pr_immediate_type = type;
					//memcpy(&pr_immediate.cppvector, (void*)(pr_globals + def->ofs), sizeof(vec));
					//memcpy(pr_globals + def->ofs, &pr_immediate, 4 * type_size[pr_immediate_type->type]);
					break;
				}
			}
