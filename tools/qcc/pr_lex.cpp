...
#if 0
int		type_size[8] = {1,1,1,3,1,1,1,1};	// Missi: already defined in pr_edict.cpp (12/3/2022)
#endif
...
type_t *PR_ParseType (void)
{
	...
	if (PR_Check ("."))
	{
		...
		prnew.type = ev_field;
		prnew.aux_type = PR_ParseType ();
		return PR_FindType (&prnew);
	}
...
// function type
	memset (&prnew, 0, sizeof(prnew));
	prnew.type = ev_function;
	prnew.aux_type = type;	// return type
	prnew.num_parms = 0;
	if (!PR_Check (")"))
	{
		if (PR_Check ("..."))
			prnew.num_parms = -1;	// variable args
		else
			do
			{
				type = PR_ParseType ();
				name = PR_ParseName ();
				strcpy (pr_parm_names[prnew.num_parms], name);
				prnew.parm_types[prnew.num_parms] = type;
				prnew.num_parms++;
			} while (PR_Check (","));
