...
static long get_lyrics3v1_len(snd_stream_t* stream) {
    ...
    g_FileSystem->FS_fseek(&stream->fh, -len, SEEK_END);
    g_FileSystem->FS_fread(buf, 1, (len -= 9), &stream->fh); /* exclude footer */
...
static long get_musicmatch_len(snd_stream_t* stream) {
    ...
    g_FileSystem->FS_fseek(&stream->fh, -68, SEEK_END);
    g_FileSystem->FS_fread(buf, 1, 20, &stream->fh);
    ...
    for (i = 0; i < 4; ++i) {
        ...
        g_FileSystem->FS_fseek(&stream->fh, -len, SEEK_END);
        g_FileSystem->FS_fread(buf, 1, 256, &stream->fh);
    ...
    g_FileSystem->FS_fseek(&stream->fh, -(len + 4), SEEK_END);
    g_FileSystem->FS_fread(buf, 1, 4, &stream->fh); j = 0;
    if (memcmp(buf, &j, 4) != 0) return -1;
#endif
    len += (version_ofs - imgext_ofs);
    if (stream->fh.length < len) return -1;
    g_FileSystem->FS_fseek(&stream->fh, -len, SEEK_END);
    g_FileSystem->FS_fread(buf, 1, 8, &stream->fh);
    ...
    g_FileSystem->FS_fseek(&stream->fh, -(len + 256), SEEK_END);
    g_FileSystem->FS_fread(buf, 1, 256, &stream->fh);
    /* [0..9]: sync string, [30..255]: 0x20 */
    if (memcmp(buf, syncstr, 10) != 0) {
        return len;
...
static int probe_id3v1(snd_stream_t* stream, unsigned char* buf, int atend) {
    if (stream->fh.length >= 128) {
        g_FileSystem->FS_fseek(&stream->fh, -128, SEEK_END);
        if (g_FileSystem->FS_fread(buf, 1, 128, &stream->fh) != 128)
            return -1;
        if (is_id3v1(buf, 128)) {
...
static int probe_mmtag(snd_stream_t* stream, unsigned char* buf) {
    ...
    if (stream->fh.length >= 68) {
        g_FileSystem->FS_fseek(&stream->fh, -48, SEEK_END);
        if (g_FileSystem->FS_fread(buf, 1, 48, &stream->fh) != 48)
            return -1;
...
static int probe_apetag(snd_stream_t* stream, unsigned char* buf) {
    ...
    if (stream->fh.length >= 32) {
        g_FileSystem->FS_fseek(&stream->fh, -32, SEEK_END);
        if (g_FileSystem->FS_fread(buf, 1, 32, &stream->fh) != 32)
            return -1;
        if (is_apetag(buf, 32)) {
            len = get_ape_len(buf);
            if (len >= stream->fh.length) return -1;
...
static int probe_lyrics3(snd_stream_t* stream, unsigned char* buf) {
    ...
    if (stream->fh.length >= 15) {
        g_FileSystem->FS_fseek(&stream->fh, -15, SEEK_END);
        if (g_FileSystem->FS_fread(buf, 1, 15, &stream->fh) != 15)
            return -1;
        ...
        if (len == 2) {
            ...
            if (len < 15) return -1;
            g_FileSystem->FS_fseek(&stream->fh, -len, SEEK_END);
            if (g_FileSystem->FS_fread(buf, 1, 11, &stream->fh) != 11)
                return -1;
...
int mp3_skiptags(snd_stream_t* stream)
{
    ...
    readsize = g_FileSystem->FS_fread(buf, 1, 128, &stream->fh);
    if (!readsize || g_FileSystem->FS_ferror(&stream->fh)) goto fail;
...
fail:
    ...
    g_FileSystem->FS_rewind(&stream->fh);
    return rc;
}
#endif /* USE_CODEC_MP3 */
