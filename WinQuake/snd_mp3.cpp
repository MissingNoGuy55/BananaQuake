...
static int mp3_inputdata(snd_stream_t *stream)
{
	...
	bytes_read = g_FileSystem->FS_fread(p->mp3_buffer + remaining, 1,
...
static int mp3_startread(snd_stream_t *stream)
{
	 ...
	 */
	ReadSize = g_FileSystem->FS_fread(p->mp3_buffer, 1, MP3_BUFFER_SIZE, &stream->fh);
	if (!ReadSize || g_FileSystem->FS_ferror(&stream->fh))
		return -1;
...
static int mp3_madseek(snd_stream_t *stream, unsigned long offset)
{
	...
	while (1)	/* Read data from the MP3 file */
	{
		...
		bytes_read = g_FileSystem->FS_fread(p->mp3_buffer + leftover, (size_t) 1,
		...
		while (1)	/* Decode frame headers */
		{
			...
			if (p->FrameCount == 64 && !vbr)
			{
				...
				if (0 != g_FileSystem->FS_fseek(&stream->fh, (p->FrameCount * consumed / 64), SEEK_SET))
					return -1;
