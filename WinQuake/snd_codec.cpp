...
snd_stream_t* S_CodecUtilOpen(const char* filename, snd_codec_t* codec, bool loop)
{
	...
	length = (long)g_Common->COM_FOpenFile(filename, &handle, NULL);
 	pak = g_Common->file_from_pak;
	if (length == -1)
	{
	...
	stream = mainzone->Z_Malloc<snd_stream_t>(sizeof(snd_stream_t));
	stream->codec = codec;
	stream->loop = loop;
