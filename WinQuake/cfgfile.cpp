...
// Missi: .cfg file handling ported from QuakeSpasm and turned into a class (1/16/2023)

#include "quakedef.h"

CGameConfig* g_GameConfig = NULL;

fshandle_t* CGameConfig::cfg_file = NULL;
...
===================
*/
void CGameConfig::CFG_ReadCvars (const char **vars, int num_vars)
{
	char	buff[1024], *tmp;
	int			i, j;
	...
	do {
		...
		if (g_FileSystem->FS_fgets(buff, sizeof(buff), cfg_file))
		{
			...
			for (i = 0; i < num_vars && vars[i]; i++)
			{
				...
				tmp = strstr(buff, g_Common->va("%s ",vars[i]));
				if (tmp != buff)
					continue;
	...
	} while (!g_FileSystem->FS_feof(cfg_file) && !g_FileSystem->FS_ferror(cfg_file));

	g_FileSystem->FS_rewind (cfg_file);
}
...
===================
*/
void CGameConfig::CFG_ReadCvarOverrides (const char **vars, int num_vars)
{
	char	buff[64];
	int		i, j;
	...
	for (i = 0; i < num_vars; i++)
	{
		...
		j = g_Common->COM_CheckParm(buff);
		if (j != 0 && j < g_Common->com_argc - 1)
		{
			if (g_Common->com_argv[j + 1][0] != '-' && com_argv[j + 1][0] != '+')
				Cvar_Set(vars[i], com_argv[j + 1]);
		}
	}
}

void CGameConfig::CFG_CloseConfig (void)
{
	if (cfg_file)
	{
		g_FileSystem->FS_fclose(cfg_file);
		mainzone->Z_Free(cfg_file);
		cfg_file = NULL;
	}
}

int CGameConfig::CFG_OpenConfig (const char *cfg_name)
{
	FILE	*f;
	long		length;
	bool	pak;
	...
	cfg_file = (fshandle_t *) mainzone->Z_Malloc<fshandle_t>(sizeof(fshandle_t));
	cfg_file->file = f;
