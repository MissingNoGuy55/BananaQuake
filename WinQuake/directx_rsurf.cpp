...
===============
*/
texture_t * CDXRenderer::R_TextureAnimation (texture_t *base)
{
	int		reletive;
	int		count;
	...
	count = 0;	
	while ((base) && (base->anim_min > reletive || base->anim_max <= reletive)) // Missi: shut up compiler (11/27/2022)
	{
		base = base->anim_next;
		if (!base)
...
================
*/
void CDXRenderer::R_DrawSequentialPoly (msurface_t *s)
{
	dxpoly_t	*p;
	float		*v;
	int			i;
	...
	if (!(s->flags & (SURF_DRAWSKY | SURF_DRAWTURB | SURF_UNDERWATER)))
	{
		...
		if (gl_mtexable) {
			...
			DX_Bind (t->gltexture);
			DX_Bind (lightmap_textures[s->lightmaptexturenum]);
	...
	if (s->flags & SURF_DRAWTURB)
	{
		...
		DX_Bind (s->texinfo->texture->gltexture);
		EmitWaterPolys (s);
		return;
	}
...
================
*/
void CDXRenderer::DrawDXWaterPoly (dxpoly_t *p)
{
	int		i;
	float	*v;
	float	s, t, os, ot;
...
void CDXRenderer::DrawDXWaterPolyLightmap (dxpoly_t *p)
{
	int		i;
	float	*v;
	float	s, t, os, ot;
...
================
*/
void CDXRenderer::DrawGLPoly (glpoly_t *p)
{
	int		i;
	float	*v;
...
================
*/
void CDXRenderer::R_BlendLightmaps (void)
{
	int			i, j;
	dxpoly_t	*p;
	float		*v;
	...
	if (dx_lightmap_format == GL_LUMINANCE)
		glBlendFunc (GL_ZERO, GL_ONE_MINUS_SRC_COLOR);
	else if (dx_lightmap_format == GL_INTENSITY)
	{
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		glColor4f (0,0,0,1);
		glBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	}
	else if (dx_lightmap_format == GL_RGBA)
	{
	...
	for (i=0 ; i<MAX_LIGHTMAPS ; i++)
	{
		...
		if (!p)
			continue;
		g_GLRenderer->DX_Bind(lightmap_textures[i]);
		if (lightmap_modified[i])
		{
			lightmap_modified[i] = false;
			theRect = &lightmap_rectchange[i];
...
================
*/
void CDXRenderer::R_RenderBrushPoly (msurface_t *fa)
{
	texture_t	*t;
	byte		*base;
	int			maps;
	...
	g_GLRenderer->DX_Bind (t->gltexture);
...
================
*/
void CDXRenderer::R_RenderDynamicLightmaps (msurface_t *fa)
{
	texture_t	*t;
	byte		*base;
	int			maps;
...
================
*/
void CDXRenderer::R_MirrorChain (msurface_t *s)
{
	if (mirror)
		return;
	mirror = true;
...
================
*/
void CDXRenderer::R_DrawWaterSurfaces (void)
{
	int			i;
	msurface_t	*s;
	texture_t	*t;
	...
	if (!gl_texsort.value) {
		...
		for ( s = waterchain ; s ; s=s->texturechain) {
			g_GLRenderer->DX_Bind (s->texinfo->texture->gltexture);
			EmitWaterPolys (s);
		}
...
================
*/
void CDXRenderer::DrawTextureChains (void)
{
	int		i;
	msurface_t	*s;
	texture_t	*t;
...
=================
*/
void CDXRenderer::R_DrawBrushModel (entity_t *e)
{
	int			j, k;
	vec3_t		mins, maxs;
	int			i, numsurfaces;
...
// instanced model
	if (clmodel->firstmodelsurface != 0 && !dx_flashblend.value)
	{
...
================
*/
void CDXRenderer::R_RecursiveWorldNode (mnode_t *node)
{
	int			i, c, side, *pindex;
	vec3_t		rejectpt;
	mplane_t	*plane;
...
=============
*/
void CDXRenderer::R_DrawWorld (void)
{
	entity_t	ent;
	int			i;
...
===============
*/
void CDXRenderer::R_MarkLeaves (void)
{
	byte	*vis;
	mnode_t	*node;
	int		i;
...
========================
*/
void CDXRenderer::GL_CreateSurfaceLightmap (msurface_t *surf)
{
...
==================
*/
void CDXRenderer::GL_BuildLightmaps (void)
{
	int		i = 0, j = 0;
	model_t	*m = NULL;
	...
	for (i=0 ; i<MAX_LIGHTMAPS ; i++, lightmap_count++)
	{
		...
		lightmap_textures[i] = GL_LoadTexture(cl.worldmodel, name, BLOCK_WIDTH, BLOCK_HEIGHT, SRC_LIGHTMAP, lightmaps + (i * BLOCK_WIDTH * BLOCK_HEIGHT) * lightmap_bytes, (src_offset_t)lightmaps + (i * BLOCK_WIDTH * BLOCK_HEIGHT) * lightmap_bytes, TEXPREF_LINEAR | TEXPREF_NOPICMIP);
	}
