...
{
	if (f == NULL) return (-1);
	return g_FileSystem->FS_fseek((fshandle_t *)f, (long) off, whence);
}

static ov_callbacks ovc_qfs =
{
	(size_t (*)(void *, size_t, size_t, void *))	g_FileSystem->FS_fread,
...
void S_VORBIS_CodecCloseStream (snd_stream_t *stream)
{
